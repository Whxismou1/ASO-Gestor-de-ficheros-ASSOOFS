...
struct dentry *assoofs_lookup(struct inode *parent_inode, struct dentry *child_dentry, unsigned int flags)
{
    ...
    struct assoofs_dir_record_entry *record;
    record = (struct assoofs_dir_record_entry *)bh->b_data;
    for (int i = 0; i < parent_info.dir_children_count; i++)
    {
...
static int assoofs_create(struct user_namespace *mnt_userns, struct inode *dir, struct dentry *dentry, umode_t mode, bool excl)
{
    ...
    inode_info->inode_no = inode->i_no;
    inode_info->mode = mode;
    inode_info->file_size = 0;
    inode_info->i_private = inode_info;
...
int assoofs_fill_super(struct super_block *sb, void *data, int silent)
{
    ...
    root_inode = new inode(sb);
    inod_init_owner(sb.s_user_ns, root_inode, NULL, S_IFDIR);
...
struct assoofs_inode_info *assoofs_get_inode_info(struct super_block *sb, uint64_t inode_no)
{
    ...
    for (int i = 0; i < afs_sb.inodes_count; i++)
    {
